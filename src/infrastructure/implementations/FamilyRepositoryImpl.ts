...
export class FamilyRepositoryImpl implements FamilyRepository {
  async findByHerbariumTypeIdForUsers(herbariumTypeId: number): Promise<Family[]> {
    const result = await pool.query(
        "SELECT * FROM family WHERE herbarium_type_id = $1 AND is_deleted = false and status = TRUE order by name",
        [herbariumTypeId]
      );
      return result.rows;
  }
  async findByHerbariumTypeId(herbariumTypeId: number): Promise<Family[]> {
    const result = await pool.query(
      "SELECT * FROM family WHERE herbarium_type_id = $1 AND is_deleted = false order by name",
      [herbariumTypeId]
    );
    return result.rows;
  }
